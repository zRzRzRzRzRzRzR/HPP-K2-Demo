...
def add_or_update_target(
    ...
    base = 1.0 if (t_type == "direct" or distance == 0) else {1: 0.8, 2: 0.6, 3: 0.4}.get(distance, 0.2)
    numeric_edges = sum(
        1 for p in graph_paths for e in p if (e.get("raw_effect") or {}).get("estimate") is not None
    )
    ...
    if node_id not in targets_map:
        ...
        tgt["type"] = "direct" if (tgt["type"] == "direct" or t_type == "direct") else "indirect"
        tgt["distance_hops"] = min(tgt.get("distance_hops", 99), distance)
        tgt["priority_score"] = round(max(tgt.get("priority_score", 0.0), score) + 0.1, 3)
        ...
        seen = {(d["name"], d.get("class", "")) for d in tgt.get("recommended_drugs", [])}
        for d in recs:
...
def build_targets_from_input(
    ...
    drug_index: Dict[str, List[dict]],
    max_hops: int = 3,
    prefer_strong_edges: bool = False,
) -> dict:
